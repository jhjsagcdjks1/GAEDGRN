...
def mask_test_edges_general_link_prediction_train_percent(adj, train_percent=10., val_percent=5., test_percent=10.):
    """
    Task 1: General Directed Link Prediction: get Train/Validation/Test

    :param adj: complete sparse adjacency matrix of the graph
    :param train_percent: percentage of edges in training set
    ...
    positive_idx, _, _ = sparse_to_tuple(
        adj)  # positive_idx 是一个包含稀疏矩阵 adj 中所有非零元素的行索引和列索引的二维数组。 [i,j] coord pairs for all true edges
    ...
    print('选择之前的边的数量：', train_edges.shape[0])
    # 计算选择的数量，这里选择10%
    num_selected = int(np.floor(train_edges.shape[0] / (100. / train_percent)))
    # 随机选择10%的边
    # 使用 random.sample 选择 num_selected 个索引
    selected_indices = random.sample(range(len(train_edges)), num_selected)

    # 根据选中的索引获取选中的边
    train_edges = train_edges[selected_indices]
    print('选择之前的边的数量：', train_edges.shape[0])
